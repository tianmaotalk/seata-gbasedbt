...
/**
 * The Type GBase8s OperateRecognizerHolder
 *
 * @author: tianmaotalk
 */
@LoadLevel(name = JdbcConstants.GBASEDBT)
public class GBasedbtOperateRecognizerHolder implements SQLOperateRecognizerHolder {

    @Override
    public SQLRecognizer getDeleteRecognizer(String sql, SQLStatement ast) {
        return new GBasedbtDeleteRecognizer(sql, ast);
    }

    @Override
    public SQLRecognizer getInsertRecognizer(String sql, SQLStatement ast) {
        return new GBasedbtInsertRecognizer(sql, ast);
    }

    @Override
    public SQLRecognizer getUpdateRecognizer(String sql, SQLStatement ast) {
        return new GBasedbtUpdateRecognizer(sql, ast);
    }
    ...
    public SQLRecognizer getSelectForUpdateRecognizer(String sql, SQLStatement ast) {
        if (((SQLSelectStatement) ast).getSelect().getFirstQueryBlock().isForUpdate()) {
            return new GBasedbtSelectForUpdateRecognizer(sql, ast);
        }
        return null;
    }
}
