...
/**
 * The type gbase8s undo insert executor.
 *
 * @author GBasedbt
 */
public class GBasedbtUndoInsertExecutor extends AbstractUndoExecutor {
    ...
    private String generateDeleteSql(List<Row> rows, TableRecords afterImage) {
        List<String> pkNameList = getOrderedPkList(afterImage, rows.get(0), JdbcConstants.GBASEDBT).stream().map(
     ...
     */
    public GBasedbtUndoInsertExecutor(SQLUndoLog sqlUndoLog) {
        super(sqlUndoLog);
    }
