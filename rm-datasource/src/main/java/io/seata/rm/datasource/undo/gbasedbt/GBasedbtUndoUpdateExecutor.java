...
/**
 * The type gbase8s undo update executor.
 *
 * @author tianmaotalk
 */
public class GBasedbtUndoUpdateExecutor extends AbstractUndoExecutor {
    ...
    protected String buildUndoSQL() {
        ...
        String updateColumns = nonPkFields.stream().map(
            field -> ColumnUtils.addEscape(field.getName(), JdbcConstants.GBASEDBT) + " = ?").collect(
            Collectors.joining(", "));

        List<String> pkNameList = getOrderedPkList(beforeImage, row, JdbcConstants.GBASEDBT).stream().map(
            e -> e.getName()).collect(Collectors.toList());
        String whereSql = SqlGenerateUtils.buildWhereConditionByPKs(pkNameList, JdbcConstants.GBASEDBT);
     ...
     */
    public GBasedbtUndoUpdateExecutor(SQLUndoLog sqlUndoLog) {
        super(sqlUndoLog);
    }
